...
class SRFBN(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, num_steps, num_groups, upscale_factor, act_type = 'prelu', norm_type = None):
        ...
        self.conv_in1 = ConvBlock(in_channels, 2*num_features,
                                 kernel_size=5,
                                 act_type=act_type, norm_type=norm_type)
        ...
        # self.conv_o1 = ConvBlock(num_features, num_features,
                                  # kernel_size=1,
                                  # act_type=act_type, norm_type=norm_type)
        # self.conv_o3 = ConvBlock(num_features, num_features,
                                  # kernel_size=3,
                                  # act_type=act_type, norm_type=norm_type)
        # self.conv_o5 = ConvBlock(num_features, num_features,
                                  # kernel_size=5,
                                  # act_type=act_type, norm_type=norm_type)
    ...
    def forward(self, x):
        ...
        for _ in range(self.num_steps):
            ...
            # h = torch.add(inter_res, self.conv_out(self.conv_out1((self.out(h)))))
            h = self.add_mean(h)
            outs.append(h)
