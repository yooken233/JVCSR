...
class FeedbackBlock(nn.Module):
    def __init__(self, num_features, num_groups, upscale_factor, act_type, norm_type):
        ...
        # 网络的第一层加入注意力机制
       # self.ca = ChannelAttention(num_features=64)
        self.esa = ESA()
...
class ESA(nn.Module):
     ...
     def forward(self, x):
         ...
         c1_ = (self.conv1(f))
         c1 = self.conv2(c1_)
         v_max = nn.functional.max_pool2d(c1, kernel_size=7, stride=3)
         v_range = self.relu(self.conv_max(v_max))
         c3 = self.relu(self.conv3(v_range))
         c3 = self.conv3_(c3)
         c3 = nn.functional.interpolate(c3, (x.size(2), x.size(3)), mode='bilinear', align_corners=False) 
         cf = self.conv_f(c1_)
         c4 = self.conv4(c3+cf)
         m = self.sigmoid(c4)
