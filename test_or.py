...
from models.cbam_before_fb import DnCNN_Refine, SRFBN
...
def test(**kwargs):
    ...
    #denoisemodel = DnCNN_Refine()
    # denoisemodel = SKIPRR()
    # checkpoint = t.load('/home/jian/SR/SRFBN_CVPR19/best_ckp.pth')
    # denoisemodel.load_state_dict(checkpoint['state_dict'])
    ...
    for item in videos:
        ...
        print("test_data_t",test_data_t.shape)
        ...
        for i in range(test_data_t.size(0)): # test_data_t.size(0)= blk_num_h  
            for j in range(test_data_t.size(1)):# test_data_t.size(1)= blk_num_w 
                ...
                for ii in range(frames_num):
                    ...
                    #x_b = x_b[ii,:,:,:,:].to(opt.device)
                    #print("x_bzoom",x_b.shape)
                    #x_b = zoom(x_b,0.5,order=3)
                 ...
                 #unsqueeze result_frame
                    # result_frame = t.unsqueeze(result_frame,0).to(opt.device)
                   # # print("result_frame.shape",result_frame.shape)
                    # result_frame = denoisemodel(result_frame)
                    # #print("result_frame.shape222",result_frame.shape)
                 # #squeeze result_frame
                    # result_frame = t.squeeze(result_frame[-1],0)
                    result_frames[ii] = result_frame
                    
                    #print("frames_t[ii]",frames_t[ii].shape)
                    #print("result_frame",result_frame.shape)
                    
                    psnr = compare_psnr(frames_t[ii].cpu().numpy(),(result_frame*255).squeeze(0).cpu().numpy(),data_range=255)
                    ssim = compare_ssim(frames_t[ii].cpu().numpy(),(result_frame*255).squeeze(0).cpu().numpy())
                    psnr_total = psnr_total + psnr
                    ssim_total = ssim_total + ssim
                    # result_framescpu = result_frames[ii].cpu().numpy()
                    # np.savetxt('test.txt',result_framescpu,'%i')
                    #result_frame = t.unsqueeze(result_frame,0)
                    
                result_data_t[i,j,:,:,:] = result_frames
        #
        #result_data_t = upsamplingmodel(result_data_t)
        uv.array2video(result_data_t,opt.save_test_root)
        #result_data_t = result_data_t.detach().cpu().numpy()
