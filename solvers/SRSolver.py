...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if loss_type == 'l1':
                ...
                self.criterion_pix = nn.CrossEntropyLoss()
            else:
    ...
    def test1image(self,inputimg):
        #print("inputimg.shape",inputimg.shape)
        self.model.eval()
        with torch.no_grad():
            forward_func = self._overlap_crop_forward if self.use_chop else self.model.forward
            if self.self_ensemble and not self.is_train:
                SR = self._forward_x8(inputimg, forward_func)
            else:
                SR = forward_func(inputimg)
    ...
    def _overlap_crop_forward(self, x, shave=10, min_size=10000000, bic=None):
        """
