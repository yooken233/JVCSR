...
def ConvBlock1(in_channels, out_channels, kernel_size, stride=1, dilation=1, bias=True, valid_padding=True, padding=0,\
    ...
    if mode == 'CNA':
        ...
        pool = nn.MaxPool2d(2) 
        d = nn.Dropout2d(0.5)
        return sequential(p, conv, n, act, pool)
    elif mode == 'NAC':
        act = activation(act_type, inplace=False) if act_type else None
        n = norm(in_channels, norm_type) if norm_type else None
        pool = nn.MaxPool2d(2) 
        return sequential(n, act, p, conv, pool)
